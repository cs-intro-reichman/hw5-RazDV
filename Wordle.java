public class Wordle {
    ...
    public static String[] readDictionary(String filename) { //dictionary.txt
		In in = new In(filename);
        String[] stringArray = in.readAllStrings();
        return stringArray;
    }
    ...
    public static boolean containsChar(String secret, char c) {
		boolean cAppears = false;
        for (int i = 0; i < 5; i++) {
            if (secret.charAt(i) == c) cAppears = true;
        }
        return cAppears;
    }
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
		for (int i = 0; i < 5; i++){
            ...
            if (containsChar(secret, x) == false) {
                resultRow[i] = '_';
            } else {
                if (secret.charAt(i) == x) {
                    resultRow[i] = 'G';
                } else {
                    resultRow[i] = 'Y';
                } 
    ...
    public static void main(String[] args) {
        ...
        char[][] guesses = new char[MAX_ATTEMPTS][WORD_LENGTH];// 6 guesses, 5 chars
        char[][] results = new char[MAX_ATTEMPTS][WORD_LENGTH];// 6 guesses, 5 chars (G, Y, _)
